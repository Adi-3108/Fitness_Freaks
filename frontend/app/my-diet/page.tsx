...
function ResetPasswordContent() {
    ...
    const fetchUserDietPlan = async () => {
      if (typeof window !== "undefined") {
        ...
        try {
          ...
          const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/user-diet-plan?userId=${userObj.id}`);
  ...
  const sendDietPlanToEmail = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/users/send-cart`, {
        method: "POST",
