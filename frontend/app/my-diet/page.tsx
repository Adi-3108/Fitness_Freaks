...
export default function MyDietPage() {
  ...
  const sendDietPlanToEmail = async () => {
    if (!user || !user.email) {
      setEmailStatus("User email not found. Please log in again.");
      return;
    }
    try {
      ...
      if (response.ok) {
        setEmailStatus("Diet plan sent to your email!");
      } else {
  ...
  return (
    <section className="diet-section">
      ...
      <div style={{ textAlign: 'center', margin: '2rem 0' }}>
        ...
        {emailStatus && (
          <div style={{ marginTop: 12, color: emailStatus.includes('sent') ? 'green' : 'red', fontWeight: 500, fontSize: '1.5rem' }}>
            {emailStatus}
          </div>
        )}
      </div>
